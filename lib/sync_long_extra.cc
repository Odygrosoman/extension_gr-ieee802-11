...
class sync_long_extra_impl : public sync_long_extra
{

public:
    sync_long_extra_impl(unsigned int sync_length, bool log, bool debug)
        : block("sync_long_extra",
        ...
        case SYNC:
            ...
            while (i + 63 < ninput) {
                ...
                if (d_offset == SYNC_LENGTH) {
                    ...
                    // ============================================================
                    // Compute absolute STF start (continuous stream sample index)
                    // ============================================================

                    // Absolute start of the SYNC window we just analyzed
                    uint64_t window_start_abs = nread + i - SYNC_LENGTH;

                    // Absolute LTF start (where LONG correlation detected)
                    uint64_t ltf_start_abs = window_start_abs + d_frame_start;

                    // Convert to first STF sample (160 STF + 32 LTF CP = 192)
                    if (ltf_start_abs >= 192) {
                        d_stf_start_abs = ltf_start_abs - 192;
                    } else {
                        d_stf_start_abs = 0;   // avoid underflow
                    }

                    d_have_stf_start_abs = true;
        ...
        case COPY:
            while (i < ninput && o < noutput) {
                ...
                if (!rel) {
                    ...
                    if (d_have_stf_start_abs) {
                        add_item_tag(0,
                                    nitems_written(0),
                                    pmt::string_to_symbol("stf_start_abs"),
                                    pmt::from_uint64(d_stf_start_abs),
                                    pmt::string_to_symbol(name()));
                    }
                }
...
private:
    ...
    uint64_t d_stf_start_abs = 0;
    bool d_have_stf_start_abs = false;
...
sync_long_extra::sptr sync_long_extra::make(unsigned int sync_length, bool log, bool debug)
{
    return gnuradio::get_initial_sptr(new sync_long_extra_impl(sync_length, log, debug));
}

const std::vector<gr_complex> sync_long_extra_impl::LONG = {
